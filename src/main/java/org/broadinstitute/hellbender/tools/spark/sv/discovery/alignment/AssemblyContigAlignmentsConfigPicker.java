...
public class AssemblyContigAlignmentsConfigPicker {
     ...
     *
     * <p>
     *     It goes through three major steps:
     *     <ul>
     *         <li>
     ...
     *             that offers more noise than signal if performed after the gap split.
     *         </li>
     *     </ul>
     * </p>
     *
     ...
     * but only parses alignments and either keep a whole contig drop it completely.
     */
     ...
     *
     * A configuration is defined as an ordered list of alignments, where the list is a subset
     * of the original alignment of the input read.
     *
     ...
     *
     * <p>
     *     This step itself has its own primitive filtering steps done
    ...
    public static List<GoodAndBadMappings> pickBestConfigurations(final AlignedContig alignedContig,
        ...
        // annotate contig if a good mapping to non-canonical chromosome exist
     ...
     * In other words, the non-canonical chromosome captures an SV--relative to the canonical chromosomes--of relatively high population frequency.
     *
     ...
     *  then this function returns all both original {@code differentRepresentationsForOneContig}s
     */
    @VisibleForTesting
     ...
     *     </li>
     *     <li>
     *         performs a final round of cleanup by moving alignments that offers low uniqueness to bad mappings
    ...
    private static AssemblyContigWithFineTunedAlignments makeContig(final String contigName, final byte[] contigSeq,
    ...
    static GoodAndBadMappings splitGapsAndDropAlignmentContainedByOtherOnRead(final GoodAndBadMappings configuration) {
        ...
        for (int i = 0; i < count; ++i) {
            Tuple2<Boolean, AlignmentInterval> stateAndAlignment = alignmentSplitChildren.get(i);
            if ( ! stateAndAlignment._1 )
                continue;
            final AlignmentInterval one = stateAndAlignment._2;
            for (int j = 0; j < count; ++j) {
                if (j == i) continue;
                final Tuple2<Boolean, AlignmentInterval> stateAndAlignmentTwo = alignmentSplitChildren.get(j);
                if (!stateAndAlignmentTwo._1)
                    continue;
                final AlignmentInterval two = stateAndAlignmentTwo._2;
                if (AlignmentInterval.overlapOnContig(one, two) == 0)
                    continue;
                if (one.containsOnRead(two)) {
                    alignmentSplitChildren.set(j, new Tuple2<>(false, two));
                } else if (two.containsOnRead(one)) {
                    alignmentSplitChildren.set(i, new Tuple2<>(false, one));
                }
            }
        }
        ...
        for (final Tuple2<Boolean, AlignmentInterval> pair : alignmentSplitChildren) {
            if (pair._1) {
                good.addAll( Lists.newArrayList(pair._2) );
            } else {
                bad.addAll( Lists.newArrayList(pair._2) );
